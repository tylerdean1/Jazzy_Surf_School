-- Transaction ID behavior for expenses:
-- - If provided, store exactly what the user typed (trimmed).
-- - If blank/NULL, generate a random UUID string.
--
-- NOTE: This is implemented in RPCs (RPC-first admin writes).
...
-- Optional backfill for existing rows that have no transaction_id.
update public.business_expenses
set transaction_id = gen_random_uuid()::text
